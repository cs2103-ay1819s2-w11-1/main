...
/**
 * Provides a handle to a card display in the card list panel.
 */
public class CardDisplayHandle extends NodeHandle<Node> {
    ...
    public CardDisplayHandle(Node cardNode) {
        super(cardNode);
    ...
    public boolean equals(Card card) {
        return getQuestion().equals(card.getQuestion()) && getAnswer().equals(card.getAnswer()) && ImmutableMultiset
                .copyOf(getTags()).equals(ImmutableMultiset.copyOf(card.getTags().stream().map(tag -> tag.tagName)
                                                                       .collect(Collectors.toList())));
    }
}
