...
public class AddCardCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        ...
        assertParseSuccess(parser,
                           PREAMBLE_WHITESPACE + QUESTION_DESC_SUBTRACTION + ANSWER_DESC_SUBTRACTION + TAG_DESC_MOD,
                           new AddCardCommand(expectedCard));

        // multiple names - last name accepted
        assertParseSuccess(parser,
                           QUESTION_DESC_HELLO + QUESTION_DESC_SUBTRACTION + ANSWER_DESC_SUBTRACTION + TAG_DESC_MOD,
                           new AddCardCommand(expectedCard));

        // multiple phones - last phone accepted
        assertParseSuccess(parser,
                           QUESTION_DESC_SUBTRACTION + ANSWER_DESC_HELLO + ANSWER_DESC_SUBTRACTION + TAG_DESC_MOD,
                           new AddCardCommand(expectedCard));

        // multiple tags - all accepted
        Card expectedCardMultipleTags = new CardBuilder(HELLO_WORLD).withTags(VALID_TAG_MOD, VALID_TAG_SUBJECT).build();
        assertParseSuccess(parser, QUESTION_DESC_HELLO + ANSWER_DESC_HELLO + TAG_DESC_MOD + TAG_DESC_SUBJECT,
                           new AddCardCommand(expectedCardMultipleTags));
    }
    ...
    public void parse_invalidValue_failure() {
        ...
        // non-empty preamble
        assertParseFailure(parser, PREAMBLE_NON_EMPTY + QUESTION_DESC_HELLO + ANSWER_DESC_HELLO + TAG_DESC_SUBJECT
                + TAG_DESC_MOD, String.format(MESSAGE_INVALID_COMMAND_FORMAT, AddCardCommand.MESSAGE_USAGE));
    }
}
